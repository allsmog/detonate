...
| # | Module | You'll learn |
|---|--------|--------------|
| 1.1 | [PE & ELF anatomy](01-foundations/01-pe-anatomy/) | Headers, sections, imports/exports, entry point — and how Detonate's static parser reads them. ✅ **Built & tested (flagship)** |
| 1.2 | [x86/x64 assembly survival kit](01-foundations/02-assembly-survival-kit/) | Registers, calling conventions, the stack, the instructions that actually matter for RE. |
| 1.3 | [The RE toolchain](01-foundations/03-re-toolchain/) | Disassemblers (Ghidra/IDA/radare2), debuggers (x64dbg/gdb), hex editors, and how they complement the sandbox. |
| 1.4 | [File triage & hashing](01-foundations/04-file-triage/) | Hashes, fuzzy hashing (ssdeep), MIME/type detection, first-look heuristics. Maps to Detonate's submission pipeline. |
...
| # | Module | You'll learn |
|---|--------|--------------|
| 2.1 | [Strings & embedded IOCs](02-static-analysis/01-strings-and-iocs/) | ASCII/UTF-16 extraction, IOC categorization, what strings reveal and what they hide. |
| 2.2 | [Imports as behavior](02-static-analysis/02-imports-as-behavior/) | Reading the Import Address Table to predict capability (network, crypto, persistence). |
| 2.3 | [Entropy & packing detection](02-static-analysis/03-entropy-and-packing/) | Shannon entropy, per-section analysis, spotting packers before you unpack. |
| 2.4 | [Writing YARA rules](02-static-analysis/04-writing-yara-rules/) | From a sample to a robust detection rule; testing against Detonate's 26 built-in rules. |
...
| # | Module | You'll learn |
|---|--------|--------------|
| 3.1 | [Your first detonation](03-dynamic-analysis/01-detonate-first-detonation/) | Submit a Linux sample, read the process tree, syscalls, network, and file drops. ✅ **Built & tested (flagship)** |
| 3.2 | [Process trees & syscall behavior](03-dynamic-analysis/02-process-trees/) | Interpreting `strace`/clone telemetry, parent-child relationships, injection patterns. |
| 3.3 | [Network behavior & C2](03-dynamic-analysis/03-network-and-c2/) | DNS, beaconing, HTTP host extraction, reading PCAP and Suricata alerts. |
| 3.4 | [MITRE ATT&CK mapping](03-dynamic-analysis/04-mitre-attack/) | Translating raw behavior into techniques/tactics; how Detonate's 26 behavioral rules work. |
...
| # | Module | You'll learn |
|---|--------|--------------|
| 4.1 | [UPX & common packers](04-unpacking-deobfuscation/01-upx-and-packers/) | Recognizing and unpacking; manual unpacking with a debugger. |
| 4.2 | [Custom packers & unpacking stubs](04-unpacking-deobfuscation/02-custom-packers/) | Finding OEP, dumping, fixing the IAT. |
| 4.3 | [String & API obfuscation](04-unpacking-deobfuscation/03-string-api-obfuscation/) | Stack strings, XOR/RC4 string decryption, API hashing/resolution. |
| 4.4 | [Scripted deobfuscation](04-unpacking-deobfuscation/04-scripted-deobfuscation/) | Automating decryption with Python; emulation with Unicorn. |
...
| # | Module | You'll learn |
|---|--------|--------------|
| 5.1 | [Sandbox & VM detection](05-anti-analysis/01-sandbox-vm-detection/) | Timing, artifacts, CPUID, registry/MAC checks — and Detonate's anti-evasion posture. |
| 5.2 | [Debugger detection & anti-debug](05-anti-analysis/02-debugger-detection/) | `IsDebuggerPresent`, PEB checks, timing, exception tricks, and patching them out. |
| 5.3 | [Evasion in the wild](05-anti-analysis/03-evasion-in-the-wild/) | Sleep-skipping, environment keying, geofencing, and how to coax execution. |
...
| # | Module | You'll learn |
|---|--------|--------------|
| 6.1 | [Decrypting embedded config](06-config-extraction/01-decrypting-config/) | Locating and decrypting C2/config blobs from a real family. |
| 6.2 | [Building a config extractor](06-config-extraction/02-building-extractor/) | A reusable parser; integrating extraction into Detonate's pipeline. |
| 6.3 | [From artifacts to intelligence](06-config-extraction/03-artifacts-to-intelligence/) | IOC export (STIX/CSV), correlation, and writing the threat report Detonate generates. |
